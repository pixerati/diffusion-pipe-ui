...
DOWNLOAD_MODELS=${DOWNLOAD_MODELS:-"true"}  # Default if not set (hunyuan, wan)
...
if [ ! -f "$INIT_MARKER" ]; then
    ...
    if [ "$DOWNLOAD_MODELS" == "hunyuan"  ]; then
    ...
    if [ "$DOWNLOAD_MODELS" == "wan"]; then

        if [ ! -d "${MODEL_DIR}/Wan2.1-T2V-1.3B" ]; then
            huggingface-cli download Wan-AI/Wan2.1-T2V-1.3B --local-dir "${MODEL_DIR}/Wan2.1-T2V-1.3B"
        else
            echo "Skipping the model Wan2.1-T2V-1.3B download because it already exists."
        fi
